    ...
    render(){
        return(
            <nav id={style.navigate}>
            	...
            	<div className={style.menu6} onClick={()=>this.props.menubarContainer.onSelectMenu('Resource')} onMouseOver={()=>this.props.menubarContainer.onResourceOver()} onMouseOut={()=>this.props.menubarContainer.onResourceOut()}><img src='img/ic_help.svg' /><span className={style.text}>{this.props.menubarContainer.state.resource}</span></div>
		</nav>
        )
    }
}
